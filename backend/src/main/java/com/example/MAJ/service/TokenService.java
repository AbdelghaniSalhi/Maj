...
import com.example.MAJ.element.TokenData;
...
public class TokenService {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(com.example.MAJ.service.TokenService.class);
    ...
    public String createToken(String userID) {
        try {
            ...
            String token = JWT.create().withClaim("c-xsrf-token", xsrf_token).withSubject(userID).withExpiresAt(c.getTime()).sign(algorithm);
    ...
    public TokenData getTokenData(String token) {
        try {
            ...
            return new TokenData(jwt.getSubject(), jwt.getClaim("c-xsrf-token").asString(), jwt.getExpiresAt().getTime());
